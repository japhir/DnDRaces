...
#+begin_src R :results graphics file :file stat_rolls.png :height 300
  stdarray <- c(8, 10, 12:15)

  replicate(1e5, stat_roll()) %>%
     make_hist() + labs(title = "Rolling for stats using the roll 4d6 drop lowest method") +
     geom_bar(aes(x = value, y = stat(count) / sum(stat(count))), data = stdarray %>% as_tibble(), fill = "red", alpha = .2, position = "identity")
#+end_src
...
#+begin_src R :colnames yes
  sa <- list(min=min(stdarray),
        mean=mean(stdarray),
        median=median(stdarray),
        max=max(stdarray))
#+end_src
...
|----------------+-----+----------+--------+-----|
| standard array |   8 |       12 |   12.5 |  15 |
| roll for stats |   3 | 12.24936 |     12 |  18 |
